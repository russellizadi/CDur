...
class AutoExpPool(nn.Module):
    def __init__(self, outputdim=10, pooldim=1):
        ...
        self.alpha = nn.Parameter(torch.full((outputdim, ), 1, dtype=float))
        self.pooldim = pooldim

    def forward(self, logits, decision):
        scaled = self.alpha * decision  # \alpha * P(Y|x) in the paper
        print(scaled.shape, logits.shape)
...
class CDur(nn.Module):
    ...
    def forward(self, x, upsample=True):
        batch, time, dim = x.shape # [64, 501, 64]
        x = x.unsqueeze(1) # [64, 1, 512, 64]
        x = self.features(x) # [64, 128, 125, 1]
        ...
        if upsample:
            decision_time = torch.nn.functional.interpolate(
                ...
                time,
                mode='linear',
                align_corners=False).transpose(1, 2) # [64, 501, 64]
        return decision, decision_time
...
class AMN(nn.Module):
    def __init__(self, inputdim, outputdim, **kwargs):
        ...
        #self.ap = AP()
        #self.outputlayer = nn.Linear(256, outputdim)
        self.outputlayer = nn.Conv1d(256, 10, kernel_size=1, padding=0, bias=False)
        self.outputlayer.apply(init_weights)
        
    def forward(self, x, upsample=True):
        batch, time, dim = x.shape # [64, 501, 64]
        x = x.unsqueeze(1) # [64, 1, 501, 64]
        #x = self.features(x) # [64, 128, 125, 1]
